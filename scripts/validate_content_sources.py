#!/usr/bin/env python3
"""
Content Sources Validation Script (ACS)

Ported from azure-container-apps-practical-guide during P3-ACS Wave 0.
Validates that all markdown files with mermaid diagrams have proper
content_sources metadata in their frontmatter.
...
**ACS Wave 0 note**: This validator is expected to produce MANY errors on
the current ACS content (121 of 136 content_sources-bearing files use
non-canonical legacy shapes). During P3-ACS Wave 0, the CI job runs this
validator in **non-blocking / warn-only** mode via ``continue-on-error: true``.
The exit-code-1 behavior is preserved for local invocation and future
strict-mode enablement (Wave 7).
...
def validate_file(file_path: Path, verbose: bool = False) -> List[ValidationError]:
    ...
    # content_sources canonical shape is { diagrams: [...] }. A legacy list
    # shape ([{ type, url }, ...]) exists in a few generated dashboards; on
    # any page that contains a Mermaid block, the list shape is invalid
    # because it carries no per-diagram provenance, so reject it explicitly
    # rather than letting it silently report "diagrams is empty".
    if isinstance(content_sources, dict):
        ...
        errors.append(
            ValidationError(
                rel_path,
                "Legacy list-form content_sources is not valid for pages with Mermaid; migrate to content_sources.diagrams (mapping with id/type/source/...)",
            )
        )
        return errors
    else:
