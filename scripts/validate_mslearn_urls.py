#!/usr/bin/env python3
"""
MSLearn URL Validation Script (ACS)
...
**ACS Wave 0 adaptation vs. the container-apps reference**:
- The container-apps reference defaulted to iterating over ALL sibling
  ``azure-*-practical-guide`` repos in ``~/GitHub/``. That default is
  unsafe when the script lives inside an individual repo's CI (it would
  scan repos the current CI runner does not have checked out).
- ACS defaults to scanning ONLY the current repo (the one containing the
  script). Cross-repo scanning is still available via ``--project <path>``.
...
Options:
    ...
    --project   Explicit project path to validate (default: current repo)
"""
...
#
# ACS Wave 0 note: ACS's current legacy shapes include bare-string lists (§2.1 of
# the P3-ACS decomposition plan) and dict-list-without-diagrams-container variants
# (§2.2 sub-variants a/b/c). The reference-list sub-keys accepted here are
# specifically for the dict-form reference-list case, not for bare-string lists.
...
def main():
    ...
    parser.add_argument(
        ...
        "-p",
        type=str,
        help="Specific project path to validate (default: this repo)",
    )
    args = parser.parse_args()

    # ACS default: validate this repo only. The container-apps original defaulted
    # to iterating over all sibling azure-*-practical-guide repos in ~/GitHub/;
    # that default is unsafe inside per-repo CI runners which only checkout the
    # current repo. See ACS Wave 0 adaptation note in module docstring.
    if args.project:
