...
class Shot:
    ...
    @property
    def id(self) -> str:
        return cast(str, self._raw["id"])

    @property
    def file(self) -> str:
        return cast(str, self._raw["file"])

    @property
    def alt(self) -> str:
        return cast(str, self._raw["alt"])

    @property
    def diff_threshold(self) -> float:
        return float(cast(float | int, self._raw.get("diff_threshold", 0.02)))
...
class Manifest:
    def __init__(self, path: Path = MANIFEST_PATH) -> None:
        self.path: Path = path
        self._data: dict[str, Any] = cast(
            dict[str, Any], yaml.safe_load(path.read_text(encoding="utf-8"))
        )
        self._index: dict[str, dict[str, object]] = {}
        seen_files: dict[str, str] = {}
        entries = cast(list[dict[str, object]], self._data.get("screenshots", []))
        for entry in entries:
            self._validate(entry, seen_files)
            shot_id = cast(str, entry["id"])
            file = cast(str, entry["file"])
            self._index[shot_id] = entry
            seen_files[file] = shot_id
        self._pending: dict[str, dict[str, str]] = {}

    def _validate(self, entry: dict[str, object], seen_files: dict[str, str]) -> None:
    ...
    def save(self) -> None:
        ...
        ryaml.width = 4096
        doc = cast(dict[str, Any], ryaml.load(self.path.read_text(encoding="utf-8")))
        for entry in cast(list[dict[str, Any]], doc.get("screenshots", [])):
