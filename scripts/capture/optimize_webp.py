...
def main() -> int:
    ...
    _ = parser.add_argument("source", type=Path, help="Source PNG to encode")
    ...
    size = encode(source, shot.asset_path, manifest.target_width, manifest.webp_quality)
    manifest.set_captured(shot_id, _dt.date.today().isoformat())
    manifest.save()
