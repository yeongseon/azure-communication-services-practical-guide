...
| `IsHardBounce` | String flag (documented type: string) indicating whether the failure is a permanent (hard) bounce. Use `tolower(IsHardBounce) == "true"` so the predicate matches regardless of casing (empirical rows in `law-acs-email-lab` are Pascal-cased). Populated for `Bounced` status only. |
...
* **Hard Bounces vs. Soft Bounces**: Monitor `IsHardBounce` (string per the documented schema — use `tolower(IsHardBounce) == "true"` to match any casing) to identify invalid addresses that should be removed from mailing lists.
