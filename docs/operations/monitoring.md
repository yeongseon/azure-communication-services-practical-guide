...
The capture above shows a 24-request spike at the right edge of the chart — that is the test burst that also appears in the [Email Delivery Checklist KQL view](../troubleshooting/first-10-minutes/email-delivery.md#key-kql-queries) as 31 status-update rows on `ACSEmailStatusUpdateOperational`. The metric and the log are different telemetry surfaces: the metric aggregates `SendEmail` API requests on the Portal Metrics pipeline, while status updates record per-recipient *lifecycle transitions* (a single send produces several status rows as the message moves through queueing, dispatch, and terminal states). Reconcile the two only loosely — they answer different questions.

!!! tip "Need KQL instead of Portal Metrics?"
